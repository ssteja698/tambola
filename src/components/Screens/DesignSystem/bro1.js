...
const items = [1, 12, 41, 63, 81, 15, 27, 36, 54, 75, 9, 18, 48, 67, 90];
...
const GamePage = () => {
  ...
  const { userName, roomCode } = router.query;
  ...
  useEffect(() => {
    if (socket && userName && roomCode) {
      // socket.emit("leave_room", { name: userName, room: roomCode });
      // socket.emit("new-user", { name: userName, room: roomCode });
      console.log(userName, users, "*167");
  ...
  useEffect(() => {
    if (!number && isFirstTime) {
      console.log(number, isFirstTime, "*218");
  ...
  const renderTicket = () => {
    return (
      <table
        style={{ border: "2px solid black !important", width: "fit-content" }}
        className="table table-bordered border border-dark"
      >
        <thead>
          <tr>
            <th colSpan="9">Ticket No: 1</th>
          </tr>
        </thead>
        <tbody>
          {Array(3)
  ...
  return (
    <div className="game-page">
      <div className="px-3 gf-gradient-bg">
        <h1 className="pt-2">Welcome to Tambola</h1>
        <p>Get ready to play and win big!</p>
        <i className="icon fas fa-gamepad"></i>
        <div className="text animation">Lets Play!</div>
      </div>
